...
object WriteQueries {
    ...
    fun <M : MacrosEntity<M>> insertObjects(ds: MacrosDatabase, objects: Collection<M>, withId: Boolean): Int {
        return ds.insertObjectData(objects.map { it.data }, withId)
    }

    @Throws(SQLException::class)
    fun <M : MacrosEntity<M>> updateObjects(ds: MacrosDatabase, objects: Collection<M>): Int {
        return ds.updateObjects(objects)
    }

    @Throws(SQLException::class)
    fun <M : MacrosEntity<M>> deleteObject(ds: MacrosDatabase, o: M): Int {
        return ds.deleteById(o.table, o.id)
    }

    // TODO make this the general one
    @Throws(SQLException::class)
    fun <M : MacrosEntity<M>> deleteObjects(ds: MacrosDatabase, objects: Collection<M>): Int {
    ...
    @Throws(SQLException::class)
    fun <M : MacrosEntity<M>> deleteObjectsById(ds: MacrosDatabase, table: Table<M>, ids: Collection<Long>): Int {
    ...
    @Throws(SQLException::class)
    fun <M : MacrosEntity<M>> saveObject(ds: MacrosDatabase, o: M): Int {
        return saveObjects(ds, listOf(o), o.objectSource)
    }

    @Throws(SQLException::class)
    fun <M : MacrosEntity<M>> saveObjects(ds: MacrosDatabase, objects: Collection<M>, objectSource: ObjectSource): Int {
